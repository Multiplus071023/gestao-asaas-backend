...
function parsear(body) {
  try {
    ...
    if (j.chaveAcesso || j.idDps || j.nNFSe || j.numero || j.numeroNFSe) {
      ...
      // Formato chave (50 chars): cMun(7)+tpAmb(1)+tpInsc(1)+CNPJ(14)+nNFSe(13)+cNFSe(8)+rest(6)
      // nNFSe está nas posições 23-35 (13 dígitos)
      const nNFSe = j.nNFSe || j.numero || j.numeroNFSe ||
        (chave.length >= 36 ? String(+chave.slice(23, 36)) : "") || "";
      return {
        sucesso: true,
        numeroNFSe: nNFSe,
        chaveAcesso: chave,
