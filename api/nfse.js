...
// ─── Assina DPS com RSA-SHA256 e C14N-WithComments ────────────────────────────
function assinarDPS(xmlStr, certBase64, certSenha) {
  ...
  // Extrai Id do infDPS para referência
  ...
  // SignedInfo com RSA-SHA256 e C14N-WithComments
  const signedInfo =
  ...
  // Injeta assinatura antes do fechamento de </DPS>
  return xmlStr.replace(`</DPS>`, sigBlock + `\n</DPS>`);
}
...
function gzipB64(str) {
  return new Promise((res, rej) =>
    zlib.gzip(Buffer.from(str, "utf8"), (e, b) => e ? rej(e) : res(b.toString("base64")))
  );
}
...
function parsear(body) {
  try {
    ...
    if (j.chaveAcesso || j.numero || j.numeroNFSe)
      return { sucesso: true, numeroNFSe: j.numero || j.numeroNFSe || "", chaveAcesso: j.chaveAcesso || "" };
...
module.exports = async function handler(req, res) {
  ...
  let xmlAssinado;
  try { xmlAssinado = assinarDPS(xmlDPS, certificado.base64, certificado.senha); }
  ...
  // Log XML for debugging (remove in production)
  try {
    const decoded = Buffer.from(gzB64, "base64");
    const xmlLog = await new Promise((res, rej) => zlib.gunzip(decoded, (e, b) => e ? rej(e) : res(b.toString("utf8"))));
    console.log("[nfse] XML enviado (primeiros 600 chars):", xmlLog.slice(0, 600));
  } catch(e) { console.log("[nfse] Não foi possível decodificar XML para log:", e.message); }
