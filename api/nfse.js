...
function assinarXML(xmlStr, pem) {
  ...
  // Log canonical form com análise de caracteres especiais
  console.log("[nfse] === CANONICAL infDPS ===");
  console.log(canonInfDPS);
  console.log("[nfse] === FIM CANONICAL ===");
  // Detecta espaços e caracteres especiais problemáticos
  const problemChars = [];
  for (let i=0; i<canonInfDPS.length; i++) {
    const c = canonInfDPS.charCodeAt(i);
    if (c < 9 || (c > 10 && c < 13) || (c > 13 && c < 32) || c === 127) {
      problemChars.push({ pos: i, char: c, context: canonInfDPS.slice(Math.max(0,i-10), i+10) });
    }
  }
  if (problemChars.length > 0) {
    console.log("[nfse] ⚠️ CARACTERES PROBLEMÁTICOS:", JSON.stringify(problemChars));
  } else {
    console.log("[nfse] ✅ Sem caracteres especiais problemáticos");
  }
  // Detecta espaços duplos ou tabs
  const hasDoubleSpace = /  /.test(canonInfDPS);
  const hasTab = /	/.test(canonInfDPS);
  const hasNewline = /
/.test(canonInfDPS);
  console.log("[nfse] Espaços duplos:", hasDoubleSpace, "| Tabs:", hasTab, "| Newlines:", hasNewline);
  console.log("[nfse] DigestValue:", digest);

  // 2. Monta SignedInfo (standalone, sem parent namespace)
