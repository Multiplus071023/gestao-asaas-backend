...
function assinarXML(xmlStr, pem) {
  ...
  const idMatch = xmlStr.match(/CanNFSe Id="([^"]+)"/);
  ...
  const node   = doc.getElementsByTagName("CanNFSe")[0];
  if (!node) throw new Error("CanNFSe não encontrado no DOM");
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>\n` +
    xmlSemProlog.replace("</PedCanNFSe>", `\n${sigBlock}\n</PedCanNFSe>`);
}
...
module.exports = async function handler(req, res) {
  ...
  const idCan = `Can${chaveAcesso}`;

  const xmlCan = `<?xml version="1.0" encoding="UTF-8"?>
<PedCanNFSe versao="1.00" xmlns="http://www.sped.fazenda.gov.br/nfse">
  <CanNFSe Id="${idCan}">
    <tpAmb>${tpAmb}</tpAmb>
  ...
  const reqBody  = JSON.stringify({ pedCanNFSeXmlGZipB64: gzB64 });
  const buf      = Buffer.from(reqBody, "utf8");

  return new Promise((resolve) => {
    const apiReq = https.request({
      hostname, path: "/SefinNacional/nfse/cancela", method: "POST",
      ...
      apiRes.on("end", () => {
        console.log("[cancel] HTTP:", apiRes.statusCode, "| Body:", data.slice(0, 300));
        try {
